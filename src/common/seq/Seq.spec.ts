...
describe('Seq', function() {
  ...
  describe('sortByManualOrder', function() {
    const createQuestions = () => seq([
      {type: 'text'},
    ...
    it('should sort according to the manual order array', function() {
      expect(createQuestions().sortByManual(_ => _.type, order).get()).deep.eq([
        {type: 'begin_repeat'},
        {type: 'end_repeat'},
        {type: 'select_multiple'},
        {type: 'text'},
    ...
    it('should keep unknown types last', function() {
      const withUnknown = seq([
        {type: 'foo'},
        {type: 'begin_repeat'},
        {type: 'text'},
      ])

      expect(withUnknown.sortByManual(_ => _.type, order).get()).deep.eq([
        {type: 'begin_repeat'},
        {type: 'text'},
        {type: 'foo'},
      ])
    ...
    it('should handle empty order gracefully', function() {
      const emptyOrder: string[] = []
      expect(createQuestions().sortByManual(_ => _.type, emptyOrder).get()).deep.eq([
        {type: 'text'},
        {type: 'select_multiple'},
        {type: 'begin_repeat'},
        {type: 'calculate'},
  ...
  describe('percent', function() {
    it('should compute percentage', function() {
      ...
      expect(arr.percent(_ => _.age === 35)).eq(0.25)
      expect(
        arr.percent(
          _ => _.age === 35,
          _ => _.age >= 30,
      ...
      expect(arr.percent(_ => _.age < 31)).eq(0.75)
    })
  })
