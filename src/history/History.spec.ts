...
const currentDatePlusOneSec = new Date(currentDate.getTime() + 1000);
const specFilePath = 'history_spec/history.csv';
const record: CustomRecord = { key: 'test', date: currentDate.toISOString(), custom: '1' };
const record2: CustomRecord = { key: 'test2', date: currentDatePlusOneSec.toISOString(), custom: '2' };
...
describe('History (as a service)', () => {
  ...
  describe('write and read to an history', () => {
    ...
    it('should save a record', () => {
      const history = new History(specFilePath);
      history.addRecord(record);
    });

    it('should build an history from a path and fetch previous record', () => {
      const history = new History(specFilePath);
      const r = history.getLastRecord();
      expect(r).not.to.be.undefined;
    });
